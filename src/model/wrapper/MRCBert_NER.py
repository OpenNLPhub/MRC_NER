...
class MRCBert_NER(BaseWrapper):
    def __init__(self,vocab_size,num_labels,use_pretrained=True):
        super(MRCBert_NER,self).__init__()
        if use_pretrained:
            self.model=MRCBert(num_labels=num_labels)
        else:
            self.model=MRCBert(num_labels=num_labels,vocab_size=vocab_size,\
    ...
    def _cal_loss(self,batch_data,**kwargs):
        ...
        tags=torch.from_numpy(np.array(flatten_lists(tags_lists)))
